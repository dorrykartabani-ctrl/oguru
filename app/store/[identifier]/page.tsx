  ...
  const loadData = async () => {
    try {
      ...
      if (locationData) {
        ...
        // Load hours, keywords, products in parallel
    ...
    <main className="min-h-screen bg-surface">
          ...
          {business.business_types.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {business.business_types.map((type) => (
                <span
                  key={type}
                  className="inline-block px-2.5 py-0.5 bg-primary/10 text-primary text-xs font-label font-semibold uppercase tracking-wider rounded-full"
                >
                  {type}
                </span>
              ))}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span
                className={`font-semibold ${
                  openStatus.isOpen ? 'text-primary' : 'text-on-surface-variant'
                }`}
              >
                {openStatus.message}
              </span>
          ...
          <section className="mb-8">
            <div className="flex flex-wrap gap-1.5">
              {keywords.map((kw) => (
                <span
                  key={kw.id}
                  className="inline-block px-3 py-1 bg-surface-container-low text-on-surface-variant text-xs font-label font-medium rounded-full"
                >
      ...
      <section className="max-w-4xl mx-auto px-4 md:px-6 mb-10">
        <div className="flex items-center justify-between mb-4">
          ...
          {products.length > 0 && (
            <button
              onClick={() => alert('Full menu coming soon!')}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            ...
            <div className="bg-secondary-container/40 border border-secondary/20 rounded-2xl p-4 flex flex-col justify-between hover:shadow-organic transition-shadow cursor-pointer">
              <div>
                ...
                <h4 className="font-display font-semibold text-sm text-on-secondary-container">
                  Gift Voucher
                </h4>
              ...
              <button
                onClick={handleGift}
                className="mt-3 text-xs font-label font-semibold text-secondary uppercase tracking-wider text-left"
              >
                Send →
              </button>
            </div>
          ...
          {activeTab === 'hours' && (
            <div>
              {hours.length > 0 ? (
                ...
                <p className="text-on-surface-variant italic text-center py-8">
          ...
          {activeTab === 'story' && (
            <div>
              {business.description ? (
                <div className="space-y-4">
                  {business.tagline && (
                    <p className="text-lg text-on-surface font-display italic">
                ...
                <div className="space-y-3">
                  ...
                  {business.google_business_url && (
                    <a
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-container transition-colors group"
                    >
                      <div className="w-10 h-10 rounded-xl bg-white border border-outline-variant flex items-center justify-center flex-shrink-0 font-bold text-sm">
                        G
                      </div>
                      <div className="flex-1">
                        <p className="font-display font-semibold text-on-surface">
