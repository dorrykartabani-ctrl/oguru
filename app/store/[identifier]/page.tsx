...
export default function StorefrontPage({
  ...
  if (!vendor) {
    return (
      <div className="min-h-screen bg-[#f6f4eb] px-5 pt-20 text-center font-body">
        <p className="font-display text-lg font-bold">Vendor not found</p>
        <Link href="/home" className="mt-4 inline-block text-[#4a6410] underline">
          Back to Home
        </Link>
      </div>
    );
  ...
  const tradingName = vendor.trading_name ?? 'Vendor';
    ...
    <main className="min-h-screen bg-[#f6f4eb] pb-40 font-body">
      ...
      <header className="absolute top-0 left-0 right-0 z-40 flex items-center justify-between px-5 pt-12 pb-4">
        <div className="flex items-center gap-3">
          <Link
            href="/home"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f6f4eb]/80 text-[#4a6410] backdrop-blur-sm"
          >
            <ArrowLeft className="h-6 w-6" />
          </Link>
          <span className="font-display text-2xl font-extrabold text-[#4a6410] drop-shadow-sm">
            OGuru
          </span>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f6f4eb]/80 text-[#4a6410] backdrop-blur-sm">
            <SearchIcon className="h-5 w-5" />
          </button>
          <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border-2 border-white bg-teal-600 text-white shadow-sm">
      ...
      <div className="relative h-[380px] w-full">
        <img src={coverImg} alt={tradingName} className="h-full w-full object-cover" />
        ...
        <div className="absolute bottom-6 left-5 right-5">
          <div className="mb-3 inline-flex items-center gap-1.5 rounded-md bg-[#a38036] px-2 py-1 shadow-sm">
          ...
          {primaryLoc && (
            <div className="mt-2 flex items-center gap-1.5 text-white/90 drop-shadow-md">
              ...
              <span className="font-label text-sm font-medium">
                {primaryLoc.address_line_1}, {primaryLoc.suburb}
              </span>
            </div>
          )}
        </div>
      ...
      {/* ---- Category Tabs ---- */}
      <div className="sticky top-0 z-30 border-b border-[#1b1c19]/5 bg-[#f6f4eb] px-5 py-3 shadow-[0_4px_10px_rgba(0,0,0,0.02)]">
        ...
        {categories.map((cat) => (
          <div key={cat} className={activeCategory === cat ? 'block' : 'hidden'}>
            <div className="mb-4 flex items-center gap-2">
              <span className="text-xl text-[#4a6410]">☕</span>
                  ...
                  <div key={product.id} className="relative border-b border-[#1b1c19]/10 pb-6">
                    <div className="flex items-start justify-between pr-12">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 pr-4 text-sm text-[#44483a]/80 leading-relaxed">
                          {product.description}
                        </p>
                      </div>
                      <span className="shrink-0 font-display text-base font-bold text-[#924700]">
                    ...
                    <button
                      ...
                      className="absolute right-0 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-[#4a6410]/10 text-[#4a6410] transition hover:bg-[#4a6410]/20 active:scale-95"
                    >
      ...
      <div className="mt-12 flex flex-col items-center px-5 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-black shadow-lg">
          <span className="font-display text-3xl font-extrabold text-blue-400">G</span>
        </div>
        <h3 className="mt-4 font-display text-xl font-extrabold text-[#4a6410]">
          OGuru Verified Vendor
        </h3>
        <p className="mt-2 max-w-sm text-sm text-[#44483a]/80">
